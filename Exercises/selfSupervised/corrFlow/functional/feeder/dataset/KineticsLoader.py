...
def create_frames(ipath, url, frames_per_second=6, start=0, end=10):
    ...
    while True:
        fps = 0;success = True
        while (success and fps <= frames_per_second):
            capture.set(cv2.CAP_PROP_POS_MSEC,( count*1000 + (fps*1000.0/frames_per_second) ))
            iname = os.path.join(ipath,'image_%d_%d.jpg' % (count, fps))
            success, image = capture.read()
            cv2.imwrite(iname, image)  # save frame as JPEG file
            fps += 1
            #print(capture.get(0))
        if count == end:
            break
...
def read_videos(datapath, jsonpath):
    f = open(jsonpath); count = 0
    data = json.load(f)
    for i in data.items():
        ...
        vlabel = str(i[0]).strip()
        label = i[1]['annotations']['label']
        url   = i[1]['url']
        start = i[1]['annotations']['segment'][0]
        end   = i[1]['annotations']['segment'][1]
        type  = i[1]['subset']


        classpath = os.path.join(datapath, type)
        if not os.path.exists(classpath):
            os.makedirs(classpath)
        labelpath = os.path.join(classpath, label)
        if not os.path.exists(labelpath):
            os.makedirs(labelpath)
        vlabelpath = os.path.join(labelpath, vlabel)
        if not os.path.exists(vlabelpath):
            os.makedirs(vlabelpath)
        create_frames(vlabelpath, url, 6, start, end)
        count +=1
